    ...
    <div className="cv-print-area">
      ...
      <div className="fixed inset-0 z-50 overflow-y-auto bg-neutral-950/85 backdrop-blur-sm flex items-center justify-center p-2 sm:p-4 print:static print:bg-white print:overflow-visible">
        <div className="relative bg-white text-neutral-900 rounded-lg w-full max-w-4xl max-h-[92vh] overflow-y-auto shadow-2xl border border-neutral-300 print:border-none print:shadow-none print:max-h-none print:rounded-none print:w-full print:max-w-none">
          ...
          {/* CV Content */}
          <div style={{ fontFamily: "'Inter', sans-serif" }} className="cv-content p-8 sm:p-12 print:p-[18mm] space-y-5 text-[13px] text-neutral-800 leading-relaxed">
            {/* Header */}
            <div className="border-b-2 border-neutral-900 pb-3">
              <h1 className="text-2xl font-bold uppercase tracking-wide text-neutral-950" style={{ fontFamily: "'Inter', sans-serif" }}>
                {contactInfo.name}
              </h1>
              <p className="text-[13px] font-semibold uppercase tracking-widest text-neutral-600 mt-1">
                {contactInfo.title}
              </p>
            ...
            <section className="space-y-1.5">
              <h2 className="text-[12px] font-bold uppercase tracking-wider text-neutral-950 border-b border-neutral-300 pb-0.5">
                Ringkasan Profesional
              </h2>
              <p className="text-[12px] leading-relaxed text-neutral-700 text-justify">
                {contactInfo.summary}
              </p>
            </section>
            ...
            <section className="space-y-3">
              ...
              {workExperiences.map((exp) => (
                <div key={exp.id} className="space-y-1 text-[12px]">
                  <div className="flex flex-col sm:flex-row sm:items-baseline justify-between font-bold text-neutral-950">
                    ...
                    <span className="text-neutral-600 text-[11px] font-medium">{exp.period}</span>
                  </div>
            ...
            <section className="space-y-2">
              ...
              {educations.map((edu) => (
                <div key={edu.id} className="space-y-0.5 text-[12px]">
                  <div className="flex flex-col sm:flex-row sm:items-baseline justify-between font-bold text-neutral-950">
                    <span>{edu.institution}</span>
                    <span className="text-neutral-600 text-[11px] font-medium">{edu.period}</span>
                  </div>
                  <div className="text-neutral-700">
                    {edu.degree} {edu.gpa && <span className="font-semibold">(IPK: {edu.gpa})</span>}
                  </div>
                  {edu.description && (
              ...
              <div className="grid grid-cols-1 gap-1 text-[12px]">
                {skillGroups.map((sg) => (
                  <div key={sg.category} className="flex flex-col sm:flex-row sm:items-baseline gap-0.5 sm:gap-2">
                    <span className="font-bold text-neutral-900 w-44 shrink-0">{sg.category}</span>
            ...
            <section className="space-y-1.5">
              ...
              <div className="space-y-0.5 text-[12px] text-neutral-700">
